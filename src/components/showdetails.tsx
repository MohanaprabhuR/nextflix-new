...
import VideoDetails from "@/components/videoCard/videoInfo";
...
export default function ShowDetails({ show, initialData }: ShowDetailsProps) {
  const { id } = useParams();
  ...
  return (
    <div className="animate-fade-in-up bg-white max-w-[1008px]  w-full rounded-[16px_16px_0px_0px] overflow-hidden mx-auto pb-40  max-sm:pb-20 shadow-xl mt-16 max-lg:max-w-[90%] max-sm:max-w-[92%]">
          ...
          <div className="px-10 max-sm:px-4 max-lg:px-6">
            <div className="flex gap-[0_20px] w-[200px]">
              {Array.from({ length: 2 }).map((_, index) => (
                <Skeleton key={index} height="h-6" width="w-[100px]" />
              ))}
            </div>
          </div>
        ) : seasons.length ? (
              ...
              {seasons.map((season: string) => (
                <button
                  ...
                  className={`text-black text-xl font-[660] leading-[102%] tracking-[0.3px] max-sm:text-base ${
                    selectedSeason === season ? "opacity-100" : "opacity-[0.35]"
            ...
            <div className="embla__container flex gap-[0_9px] is-draggable">
              {isLoading
                ? Array.from({ length: 4 }).map((_, index) => (
                    <div key={index} className="w-full max-w-[296px] flex-none">
